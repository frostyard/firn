...
# Build pipeline with version info
build-pipeline:
    cd pipeline && go build -ldflags="-X github.com/frostyard/firn/pipeline/internal/version.Version=$(git describe --tags --always --dirty 2>/dev/null || echo dev) -X github.com/frostyard/firn/pipeline/internal/version.Commit=$(git rev-parse --short HEAD 2>/dev/null || echo none) -X github.com/frostyard/firn/pipeline/internal/version.Date=$(date -u +%Y-%m-%dT%H:%M:%SZ) -X github.com/frostyard/firn/pipeline/internal/version.BuiltBy=local" -o bin/pipeline ./cmd/pipeline/
